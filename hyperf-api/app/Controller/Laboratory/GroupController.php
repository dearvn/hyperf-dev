...
class GroupController extends AbstractController
{
    ...
    public function historyMessage()
    {
        ...
        if (empty($contactId)) $this->throwExp(StatusCode::ERR_VALIDATION, 'ID the parameters are not allowed to be empty');
    ...
    {
        $contactId = $this->request->query('contact_id') ?? '';
        if (empty($contactId)) $this->throwExp(StatusCode::ERR_VALIDATION, 'Group ID parameters are not allowed to be empty');
    ...
    /**
     * Obtaining group members management list
    ...
    public function groupMemberManage()
    {
        $contactId = $this->request->query('contact_id') ?? '';
        if (empty($contactId)) $this->throwExp(StatusCode::ERR_VALIDATION, 'Group ID parameters are not allowed to be empty');
        if (empty($groupInfo = Group::findById($contactId))) $this->throwExp(StatusCode::ERR_EXCEPTION, 'The group does not exist');
