    ...
    <div class="forward-message" @click="catForwardRecords">
      ...
      <div v-for="(message, index) in content.message" :key="index" class="lists">
        <p v-if="index < 2">
          ...
          <span v-if="message.type == 'file'">[File Message]</span>
          <span v-if="message.type == 'image'">[Image message]</span>
          <span v-if="message.type == 'video'">[Video message]</span>
        </p>
      </div>
      <div class="footer">
        <span>Forward: chat session record ({{ content.total }}strip)</span>
      </div>
    </div>
    <!--session log viewer-->
...
export default {
  ...
  methods: {
    ...
    getForwardTitle(list) {
      ...
      return arr.join('、') + 'Session record'
    },
  },
  created() {
