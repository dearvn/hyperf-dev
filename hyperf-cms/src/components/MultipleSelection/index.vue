  ...
  >
    <div class="filter-container">
      <div class="all-fields">
        ...
        >
          <span>all</span>
          <el-button type="danger" size="mini" @click="clearSelection">Empty choice</el-button>
        </div>
      ...
      <el-button style="margin-top: 10px;float: right;" @click="closeDialog">Cancel</el-button>
      <el-button
        type="primary"
...
export default {
      ...
      fieldLoop: for (let i = 0; i < fieldLen; i++) {
        ...
        for (let j = 0; j < childLen; j++) {
          if (this.fields[i].children[j].name == checked) {
            ...
            //Take the parent id as a module, and the indicators of different modules are sorted separately, and cannot be mixed together for sorting
            if (
  ...
  methods: {
    ...
    removedFieldId(oldArr, newArr) {
      //Find out the id of the deletion
        ...
        for (let i in this.selectedFields) {
          for (let k = 0; k < this.selectedFields[i].length; ) {
            if (this.removeArr.indexOf(this.selectedFields[i][k].id) > -1) {
              ...
              //Adjust the order, delete parent indicators without child indicators
    ...
    deleteSelected(data, index) {
      ...
      //Check indicator checkbox status
      ...
      fieldLoop: for (let i = 0; i < len; i++) {
        ...
        //If no child indicator is selected in the current parent indicator, clear the selection box state
        this.fields[i].indeterminate = false
      }
